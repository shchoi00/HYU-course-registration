...
- **바로 취케팅**: 0.5초 간격으로 희망수업 정원을 조회하고 빈자리가 확인된 과목만 신청합니다.
...
모드 선택 화면에서 `바로 취케팅`을 선택하면 예약 대기 없이 정원 조회를 시작합니다. 한 번의 조회로 선택된 모든 과목의 `신청인원/제한인원`을 확인하며, 조회 라운드 사이에는 0.5초 대기합니다. 만석인 동안에는 NetFunnel 티켓이나 수강신청 요청을 보내지 않습니다.
...
4. 실행 모드 선택:
   ...
   - `바로 취케팅`: 0.5초마다 정원을 조회하고 빈자리가 확인된 과목만 신청한 뒤 신청내역을 재확인.
