...
- **바로 취케팅**: 평균 1초(`0.7~1.3초` 랜덤) 간격으로 희망수업 정원을 조회하고 빈자리가 확인된 과목만 신청합니다.
- **신청 결과 재확인**: 신청 응답만 믿지 않고 다음 희망수업 조회에서 해당 수업번호의 신청 여부를 확인합니다.
...
모드 선택 화면에서 `바로 취케팅`을 선택하면 예약 대기 없이 정원 조회를 시작합니다. 한 번의 조회로 선택된 모든 과목의 `신청인원/제한인원`을 확인하며, 조회 라운드 사이에는 매번 새로 선택한 `0.7~1.3초`의 랜덤 시간(평균 1초)을 대기합니다. 만석인 동안에는 NetFunnel 티켓이나 수강신청 요청을 보내지 않습니다.

빈자리가 확인되면 우선순위 순서로 해당 과목만 즉시 신청합니다. 서버 응답이 성공이더라도 다음 희망수업 조회에서 같은 수업번호의 신청 건수가 확인되어야 최종 완료로 처리합니다. 빈 응답은 성공으로 간주하지 않으며, 이미 다른 분반을 신청해 `중복 신청` 응답이 온 과목은 반복하지 않고 별도 중단 상태로 표시합니다.

조회 상태는 다음처럼 같은 터미널 한 줄에서 갱신됩니다.

```text
[취케팅 조회 128] COE8042 120/120 만석 | COE9016 179/180 빈자리
```
...
4. 실행 모드 선택:
   - `예약 수강신청`: 지정 시각까지 대기한 뒤 우선순위대로 과목별 1회 시도.
     - 실패한 과목만 추려 즉시 정원 조회형 `바로 취케팅`으로 전환.
   - `바로 취케팅`: 평균 1초(`0.7~1.3초` 랜덤) 간격으로 정원을 조회하고 빈자리가 확인된 과목만 신청한 뒤 신청내역을 재확인.
...
- `tk 토큰을 찾을 수 없습니다`가 나오면 최신 코드를 받은 뒤 다시 실행합니다. 현재 버전은 HTML 엔티티, 대소문자 및 길이가 달라진 URL-safe 토큰도 처리하며, 토큰이 실제로 없는 로그인 응답은 traceback 없이 인증 오류로 안내합니다.
